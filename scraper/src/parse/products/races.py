...
class RaceProduct(Product):
    """Shared event extraction for racing products (dogs, horses, speedway,
    motorbikes). Subclasses only declare which wire `classType` values map to
    them. The actual race attributes are largely interchangeable; per-product
    extras live in `EventRunner.raw` (the original participant dict)."""
        ...
        if is_data and ev0_data:
            ...
            for trap_index, p in enumerate(parts, start=1):
                ...
                runners.append(EventRunner(
                    ...
                    runner_id=runner_id,
                    trap=trap_index,
                    name=p.get("name") or p.get("rider"),
